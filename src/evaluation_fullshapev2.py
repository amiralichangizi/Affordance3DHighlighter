...
# here we can try different thresholds, any scores above threshold mean affordance present(1) else not(0)! 
def evaluate_affordance(pred_scores, gt_labels, threshold=0.3):
    """
...
# this basically creates a 3d scatter plot comparing ground truth labels vs predicted labels. (actual affordance vs predicted affordance)
# this helps visually understand wether the model focuses on correct regions or not! 
def visualize_predictions(coords, pred_scores, gt_labels, shape_id, affordance, output_dir):
    ...
    """
    sampled_idx = torch.randperm(coords.size(0))[:2000]  # Downsampling for visualization to avoid overcrowding 
    coords = coords[sampled_idx]
    pred_scores = pred_scores[sampled_idx]
    gt_labels = gt_labels[sampled_idx] # color coded regions for actual affordance
...
# evaluates the effectiveness of a single prompt strategy for a given affordance. 
def evaluate_strategy(net, clip_model, coords, gt_binary, shape_class, affordance, strategy, device):
    ...
    """
    prompt = generate_affordance_prompt(shape_class, affordance, strategy) # check the prompt strategies for a better idea
